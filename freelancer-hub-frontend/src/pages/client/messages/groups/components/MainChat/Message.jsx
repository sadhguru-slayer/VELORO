...
const Message = ({ 
  ...
  const calculateModalPosition = useCallback(() => {
    ...
    if (message.isCurrentUser || message.sender === "You") {
      // If message is on the left side of the viewport
      if (messageRect.left < viewportCenterX) {
      ...
      if (messageRect.right > viewportCenterX) {
        horizontalPosition = 'left';
      } else {
        horizontalPosition = 'right';
      }
  ...
  // Check if message is near bottom/right of viewport
  useEffect(() => {
    if (messageRef.current && containerDimensions) {
      const checkPosition = () => {
        ...
        if (messageContainer) {
          ...
          setShowAbove(rect.bottom > containerRect.bottom - 200);
          
          // Check horizontal position - for action menus
          const isCurrentUser = message.isCurrentUser || message.sender === "You";
          const rightSpace = containerRect.right - rect.right;
          const leftSpace = rect.left - containerRect.left;
          
          // If user's message (right-aligned), check if there's enough space on the left
          // If other's message (left-aligned), check if there's enough space on the right
          if (isCurrentUser) {
            setShowLeft(leftSpace < 150); // Not enough space on the left
          } else {
            setShowLeft(rightSpace < 150); // Not enough space on the right
          }
        }
      };
  ...
  }, [message.isCurrentUser, message.sender, containerDimensions]);
  ...
  const renderAttachmentPreview = (attachment) => {
    ...
    if (isImage) {
      ...
      return (
        <div className="w-full p-3 bg-teal-50 rounded-lg flex flex-col space-y-2">
          <div className="flex items-center space-x-3">
            <div className="p-2 bg-white rounded-lg flex-shrink-0 w-10 h-10 flex justify-center items-center shadow-sm">
              <FaMicrophone className="w-5 h-5 text-teal-500" />
            </div>
  ...
  return (
    ...
    >
      {/* Avatar for other users' messages (not shown for user's own messages) */}
      {!(message.isCurrentUser || message.sender === "You") && (
        <div className="flex-shrink-0 mr-2 mt-1">
          <img
            src={message.avatar || `https://ui-avatars.com/api/?name=${message.sender}&background=random`}
            alt={message.sender}
              ...
              >
                ...
                >
                  <FaThumbtack className={`w-4 h-4 ${
                    isPinned ? 'text-teal-500' : 'text-gray-500'
                  }`} />
                  <span className={isPinned ? 'text-teal-600' : 'text-gray-700'}>
                ...
                <div className="flex flex-nowrap overflow-x-auto gap-1">
                  {['👍', '❤️', '😂', '😮', '😢', '😡', '🎉', '👏'].map((emoji) => (
                    <motion.button
                      ...
                      className="p-2 hover:bg-gray-50 rounded-xl transition-all duration-200 ease-out text-2xl flex-shrink-0"
                      style={{
              ...
              return (
                <button
                  ...
                  className={`text-sm border rounded-full px-2 py-0.5 hover:bg-gray-50 transition-colors ${
                    ...
                    hasUserReacted ? 'bg-teal-100' : 'bg-white'
                  }`}
                >
